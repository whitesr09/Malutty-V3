...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('White-sr is Working😍 ' + config.WORKTYPE + ' 𝚗𝚘𝚠 🍃'));
          if (config.LANG == 'EN' || config.LANG == 'ML') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '*◄━⦁💝 WHITE-SR-V2💝⦁━►*' + degisiklikler + '```', MessageType.text
                    ); 
                } 
          }
    });//thanx afnanplk
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ WHITE-SR-V2 ~______~*' +
                                    '\n*WHITE-SR ERROR🔰' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
